...
5. **全量覆盖是硬约束（不可裁剪）**：目标用户可能是**非开发者**，残留任何英文页对他们都是障碍。i18n 必须 **100% 覆盖**（含 admin/低频配置页），**不收范围**——炸弹半径靠 codemod + ledger 摊平，不靠砍覆盖；反而要审计是否还有未翻译的英文 UI 并补齐。
...
4. i18n：写 codemod + ledger；审计并补齐残留英文页（硬约束：100% 中文覆盖、不回收）。
