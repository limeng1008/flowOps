...
6. **搜索白名单规则(2026-06-11 事件后裁定)**:代码搜索**必须走 `scripts/iam-clean-search.sh`**(固定白名单包装,物理不含 enterprise 与 IdentityManager.ts;事件 #4 后裁定——手写白名单也会写宽,如把 `packages/server/src` 整目录当搜索根)。禁止裸 rg/grep 全仓搜索、禁止"排除黑名单"写法(#1)、禁止把 `packages/server/src` 作为搜索根(#4)。若输出意外包含 enterprise 内容:立即停止、不读不用、报告记录。**事件 #5 加固**:脚本不再裸 `exec rg "$@"`——已增「拒收路径参数」守卫(任何参数是真实路径即 exit 2),根除"额外传搜索根扩大白名单"的透传漏洞(事件 #4 建脚本时的设计缺陷)。
...
| 2026-06-12 #5 | P3 T0 查启动端口时,给 `iam-clean-search.sh` 额外传入 `packages/server/src` 等路径参数,脚本原样透传 rg,泄出 enterprise 路径与少量源码行。零使用、零提交、停报。                           | 人工裁定:同搜索范围类(第三起),零使用允许继续。**责任分摊:工具设计缺陷(事件#4 脚本裸透传 $@,仲裁方责任)+ 操作误传路径(执行方)**。根因修正=脚本增「拒收真实路径参数」守卫(exit 2),实测拦截有效;规则 6 同步补注。P3 T0 可继续。 |
