...
P3 T0 audit date: 2026-06-12

Branch: `codex/iam-p3-ship`

Plan baseline: `.planning/flowops-second-dev/codex-flowops-iam-p3-ship-plan.md` at local commit `b508153f`
...
P3 seam bridges in `packages/server/src/iam/**`: 9 total.
...
Current dist target counts:

-   `packages/server/dist/enterprise/**`: 373 paths.
...
Already executed before ledger commit:
...
| enterprise diagnostic leak gate            | `awk '/src\\/enterprise\\// { c++ }' /tmp/flowops-p3-t0-tsc.log`                                                     | 0                                   |
| server jest                                | `cd packages/server && npx jest`                                                                                     | 65 suites passed, 1054 tests passed |
| server build before true-machine           | `cd packages/server && pnpm build`                                                                                   | exit 0                              |

Pending in this ledger until the final T0 run completes:

-   Dual-track true-machine startup on temporary ports with read-only health probes.
