...
const AddEditDatasetRowDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
    ...
    const addNewDatasetRow = async () => {
        try {
            ...
            if (createResp.data) {
                enqueueSnackbar({
                    message: t('pages.datasets.rowAdded'),
                    options: {
            ...
            enqueueSnackbar({
                message: t('pages.datasets.rowAddFailed', {
                    message:
    ...
    const saveDatasetRow = async () => {
        try {
            ...
            if (saveResp.data) {
                enqueueSnackbar({
                    message: t('pages.datasets.rowSaved'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('pages.datasets.rowSaveFailed', {
                    message:
                        typeof error.response?.data === 'object'
                            ? error.response?.data?.message
                            : error.response?.data || error.message || t('pages.assistants.unknownError')
            ...
            <DialogTitle sx={{ fontSize: '1rem' }} id='alert-dialog-title'>
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    {dialogProps.type === 'ADD'
                        ? t('pages.datasets.addItemTitle', { name: datasetName })
                        : t('pages.datasets.editItemTitle', { name: datasetName })}
                </div>
            </DialogTitle>
            <DialogContent>
                <Box sx={{ p: 2 }}>
                ...
                <Box sx={{ p: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'row' }}>
                        <Typography>
                            {t('pages.datasets.anticipatedOutput')}
                            <span style={{ color: 'red' }}>&nbsp;*</span>
                        </Typography>
