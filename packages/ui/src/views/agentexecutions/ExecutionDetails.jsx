...
export const ExecutionDetails = ({ open, isPublic, execution, metadata, onClose, onProceedSuccess, onUpdateSharing, onRefresh }) => {
    ...
    const copyToClipboard = () => {
        ...
        dispatch(
            enqueueSnackbarAction({
                message: t('pages.executions.idCopied'),
                options: {
    ...
    const buildTreeData = (nodes) => {
        ...
        iterationGroups.forEach((iterationMap, parentId) => {
            iterationMap.forEach((nodeIds, iterationIndex) => {
                ...
                const iterationLabel = t('pages.executions.iterationLabel', { index: iterationIndex })
    ...
    const onSharePublicly = () => {
        ...
        updateExecutionApi.request(localMetadata.id, { isPublic: newIsPublic }).then(() => {
            ...
            dispatch(
                enqueueSnackbarAction({
                    message: newIsPublic ? t('pages.executions.sharedPublicly') : t('pages.executions.noLongerPublic'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
    ...
    const resizeHandle = (
        <button
            aria-label={t('pages.executions.resizeDrawer')}
            style={{
                position: 'absolute',
                left: 0,
                top: 0,
